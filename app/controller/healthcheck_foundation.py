...
class HealthCheckFoundation:
    ...
    def mount_point_template_vertification(mount_point:dict)->bool:
        ...
        if not HealthCheckFoundation.is_valid_capacity_threshold(mount_point['threshold_percentage']) \
            or not HealthCheckFoundation.is_valid_mount_point(mount_point['mount_point']) \
                or not HealthCheckFoundation.is_valid_synonym(mount_point["synonym"]):
            return False
    ...
    def webservice_template_verification(webservice:dict)->bool:
        ...
        if not HealthCheckFoundation.is_hostname_ipv4_ipv6(webservice['hostname']) \
            or not HealthCheckFoundation.is_valid_port(webservice['port']) \
                or not HealthCheckFoundation.is_valid_web_protocol(webservice['protocol']) \
                    or not HealthCheckFoundation.is_valid_synonym(webservice["synonym"]):
            return False
    ...
    def database_template_verification(database:dict)->bool:
        ...
        if not HealthCheckFoundation.is_hostname_ipv4_ipv6(database['hostname']) \
            or not HealthCheckFoundation.is_valid_port(database['port']) \
                or not HealthCheckFoundation.is_valid_db_driver(database['database_type']) \
                    or not HealthCheckFoundation.is_valid_synonym(database["synonym"]):
            return False
        return True
    ...
    def requirements_template_verification(requirements:list[str])->bool:
        ...
        # Check if the synonym is valid
        if not HealthCheckFoundation.is_valid_file_path(requirements['requirements_file_path']) \
            or not HealthCheckFoundation.is_valid_synonym(requirements["synonym"]):
            return False
    ...
    def is_hostname_ipv4_ipv6(hostname:str)->bool:
        ...
        return HealthCheckFoundation.is_valid_hostname(hostname) or HealthCheckFoundation.is_valid_ip_address(hostname)
    ...
    def is_valid_db_driver(db_driver:str)->bool:
        ...
        return any(db_driver.lower() in drivers for drivers in HealthCheckFoundation.DB_DRIVER_MAP.values())
    ...
    def get_database_driver_by_type(database_type:str)->str:
        ...
        return HealthCheckFoundation.DB_DRIVER_MAP.get(database_type.lower(), [None])[0] if database_type else None
